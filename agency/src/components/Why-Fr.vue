...
<div v-if="questions" v-motion-slide-visible-once-bottom  class="questions-container">

<div class="questions-illustration">
  </div>

<div class="questions">
  <h1>Quelques questions que vous devriez vous poser</h1>
...
<p v-for="question in questions" :key="question">{{question.attributes.question}}</p>
  ...
  <h1 class="questions_title">Les avantages d'un site web</h1>

<div class="items_container">

<div v-for="advantage in advantages" :key="advantage" class="item"  >
      <img v-motion-slide-bottom   :src="imageUrl+advantage.attributes.icon.data.attributes.url" class="icon" alt="">
...
try{
const res = await axios.get('why?locale=fr&populate=*')
...
try{
  ...
  const res = await axios.get('questions?locale=fr')
...
try{
  ...
  const res = await axios.get('advantages?locale=fr&populate=*')
